...
<div class="bp-settingBottom">
  ...
  <button class="bp-resetSettingBtn">${i18n.settings.reset}</button>
    ...
    document.querySelectorAll('.bp-modal input[type="checkbox"]').forEach((elm) => {
      elm.checked = _GM_getValue(elm.name, elm.getAttribute("data-default") == "true" ? true : false);
    });
    document.querySelectorAll(".bp-modal select").forEach((elm) => {
      elm.value = _GM_getValue(elm.name, elm.getAttribute("data-default"));
    });
    ...
    document.querySelector(".bp-saveSettingBtn").addEventListener("click", () => {
      document.querySelectorAll('.bp-modal input[type="checkbox"]').forEach((elm) => {
        _GM_setValue(elm.name, elm.checked);
      });
      document.querySelectorAll(".bp-modal select").forEach((elm) => {
        _GM_setValue(elm.name, elm.value);
      });
  ...
  const getToken = (type, illustID, tries, svgBtnElm, restrictMode, artworksPageMode, errBypass) => {
    const url = type == "addR18" || type == "delR18" || errBypass ? "https://www.pixiv.net/bookmark_new_illust.php" : location.href;
    _GM_xmlhttpRequest({
      method: "GET",
      url,
      headers: {
      ...
      onload: (response) => {
        ...
        _GM_setValue("bkm_token", JSON.parse(data.content).token);
        _GM_setValue("bkm_tokenExpireTimestamp", Date.now() + 15 * 60 * 1e3);
  ...
  const addBookmark = (illustID, tries, svgElm2, restrictMode, artworksPageMode) => {
    ...
    _GM_xmlhttpRequest({
      ...
      onload: (response) => {
        ...
        const tags = _GM_getValue("bkm_addAllTags", true) ? [
        ...
        switch (restrictMode) {
          case 2:
            isPrivate = _GM_getValue("bkm_alwaysPrivate", false);
            if (_GM_getValue("bkm_r18Private", true) && (tags.includes("R-18") || tags.includes("R-18G"))) {
              isPrivate = true;
            }
            break;
          case 1:
        ...
        _GM_xmlhttpRequest({
          ...
          headers: {
            ...
            "x-csrf-token": _GM_getValue("bkm_token")
          },
          data: JSON.stringify({
            illust_id: illustID,
            comment: "",
          ...
          onload: (response2) => {
            ...
            if (data.error) {
              if (tries <= 2) getToken("add", illustID, tries + 1, svgElm2, restrictMode, artworksPageMode);
              return;
            }
            let svg;
            if (isPrivate) {
            ...
            svgElm2.innerHTML = svg;
            if (location.hash == "#discover") {
              svgElm2.classList.remove("hgCyhg");
              return;
            }
            if (location.pathname == "/") {
              svgElm2.classList.remove("fyVBNZ");
              return;
            }
            if (artworksPageMode) {
  ...
  const deleteBookmark = (illustID, tries, svgElm2, artworksPageMode) => {
    const url = `https://www.pixiv.net/ajax/illust/${illustID}`;
    _GM_xmlhttpRequest({
      method: "GET",
      url,
      onload: (response) => {
        ...
        _GM_xmlhttpRequest({
          ...
          onload: (response2) => {
            const data = JSON.parse(response2.responseText);
            if (data.error) {
              if (tries <= 2) getToken("del", illustID, tries + 1, svgElm2, null, artworksPageMode);
              return;
            }
            ...
            svgElm2.innerHTML = svg;
            if (location.hash == "#discover") {
              svgElm2.classList.add("hgCyhg");
              return;
            }
            if (location.pathname == "/") {
              svgElm2.classList.add("fyVBNZ");
              return;
            }
            if (artworksPageMode) {
              document.querySelector(
...
.bp-btn-like.done .icon-like.private{
  ...
  let lang = _GM_getValue("language", "auto");
  ...
  let MOManager;
  const logSwitch = _GM_getValue("sw_logOutput", false);
  const pluginSwitch = {
    updateUIStyle_html: true,
    updateUIStyle_body: true,
    settings: true,
    adRemover: _GM_getValue("sw_adRemover", true),
    searchEnhancer: _GM_getValue("sw_searchEnhancer", true),
    bigPicturePaging: _GM_getValue("sw_bigPicturePaging", true),
    bookmarkAddEnhancer: _GM_getValue("sw_bookmarkAddEnhancer", true)
  };
  const settingList = {
    general: [
      {
  ...
  let maxPageNum;
  if (_GM_getValue("bkm_token", null) !== null) {
    _GM_deleteValue("bkm_token");
  }
  ...
  let plugins = [
    ...
    [
      ...
      (mutationList, observer) => {
        for (const mutation of mutationList) {
          if (mutation.type != "childList" || document.querySelector(
          ...
          ) == null || document.querySelector(".bp-openSettingLink") != null)
            return;
...
<button>${i18n.searchEnhancer.filter}</button>`;
            ...
            numList.forEach((num) => {
              ...
              favoriteFilterUI.querySelector("select:nth-of-type(2)").innerHTML = `<option value="${num}">${num}</option>` + favoriteFilterUI.querySelector("select:nth-of-type(2)").innerHTML;
            });
          }
          if (document.querySelector(".bp-favoriteFilterUI") != null && cachedTagPagePathname != location.pathname) {
            ...
            if (regexBetween.test(decodedPathname)) {
              ...
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(1)").value = decodedPathname.match(regexBetween)[2];
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(2)").value = decodedPathname.match(regexBetween)[3];
            } else if (regexOneNum.test(decodedPathname)) {
              tag = decodedPathname.match(regexOneNum)[1];
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(1)").value = decodedPathname.match(regexOneNum)[2];
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(2)").value = decodedPathname.match(regexOneNum)[2];
            } else {
              tag = decodedPathname.match(regexNoNum)[1];
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(1)").value = 5;
              document.querySelector(".bp-favoriteFilterUI select:nth-of-type(2)").value = 8e5;
            }
            document.querySelector(".bp-favoriteFilterUI button").addEventListener("click", () => {
              ...
              if (selectedNumList[0] == selectedNumList[1]) {
                ...
                favoriteFilterStr = "(";
                for (let index = numList.findIndex((elm) => elm == selectedNumList[0]); index < numList.findIndex((elm) => elm == selectedNumList[1]); index++) {
              ...
              if (hashes["s_mode"] == "s_tc") {
                location.href = encodeURI(
                  decodeURI(location.href).replace(regexNoNum, `/tags/${tag} ${favoriteFilterStr}/`).replace("s_mode=s_tc", "s_mode=s_tag")
                );
                return;
              }
              location.href = encodeURI(
    ...
    [
      ...
      (mutationList, observer) => {
        for (const mutation of mutationList) {
          ...
          if (/#big_\d+/.test(location.hash) && !document.querySelector(".bp-bigPicturePrev") && document.querySelector("button.nav-back")) {
      ...
      (mutationList, observer) => {
        for (const mutation of mutationList) {
          ...
          document.querySelectorAll(".works-item-illust").forEach((item) => {
            ...
            customizedBtn.addEventListener("click", () => {
              if (customizedBtn.querySelector("svg").innerHTML == `<path fill-rule="evenodd" clip-rule="evenodd" d="M16 9.36771C17.2648 7.66742 19.1383 6.5 21 6.5C24.3137 6.5 27 9.18629 27 12.5C27 17.854 23.5228 22.4393 16.7322 26.2374C16.2772 26.4919 15.7228 26.4919 15.2678 26.2374C8.47724 22.4393 5 17.854 5 12.5C5 9.18629 7.68629 6.5 11 6.5C12.8617 6.5 14.7352 7.66742 16 9.36771Z" fill="white" stroke="#1F1F1F" stroke-width="2"></path>`) {
                if (_GM_getValue("bkm_token", null) && Date.now() < _GM_getValue("bkm_tokenExpireTimestamp", 0)) {
  ...
  if (ua.indexOf("android") !== -1 || ua.indexOf("iphone") !== -1 || ua.indexOf("ipad") !== -1 || _GM_getValue("OSCheckBypass", false)) {
    ...
    if (_GM_getValue("version", "0.0.0") != _GM_info.script.version) {
...
<div class="bp-modalInner">
  <p class="bp-categoryTitle">${i18n.updateModal.title.replace("%0", _GM_info.script.version)}</p>
  <p>${i18n.updateModal.body.replace("%0", _GM_info.script.version)}</p>
  ...
  <label><input type="checkbox" class="bp-updateModalCheck">${i18n.updateModal.check}</label>
      ...
      updateModal.querySelector(".bp-closeModalBtn-um").addEventListener("click", () => {
        if (updateModal.querySelector(".bp-updateModalCheck").checked) {
          _GM_setValue("version", _GM_info.script.version);
        }
        updateModal.remove();
...
<div class="bp-modalInner">
  ...
  <label><input type="checkbox" class="bp-firstRunModalCheck">${i18n.firstRunModal.check}</label>
    ...
    if (userInput && userInput.toLowerCase() == "y") {
      _GM_setValue("OSCheckBypass", true);
      if (confirm(i18n.settings.confirmReload)) {
        location.reload();
      }
    }
